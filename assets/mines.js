...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-b185e86d";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-b185e86d">'+'<span class="tile-face-b185e86d tile-front-b185e86d"></span>'+'<span class="tile-face-b185e86d tile-back-b185e86d"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-b185e86d")){return;}
var back=tile.querySelector(".tile-back-b185e86d");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-b185e86d");if(isMine){tile.classList.add("boom-b185e86d");}
if(dim){tile.classList.add("dimmed-b185e86d");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-b185e86d")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-b185e86d");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-b185e86d")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-b185e86d");void panelEl.offsetWidth;panelEl.classList.add("jolt-b185e86d");}
endRound();}
